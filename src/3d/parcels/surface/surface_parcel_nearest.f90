!==============================================================================
!           Finds the surface parcels nearest every "small" surace parcel
!==============================================================================
...
!     integer:: merge_nearest_timer, merge_tree_resolve_timer
    ...
    !Other variables:
    double precision:: delx,dely,dsq,dsqmin,x_small,y_small
    ...
    contains
        ...
        subroutine find_nearest(s_parcels, n_par, isma, iclo, nmerge)
...
!             call start_timer(merge_nearest_timer)
            ...
            do n = 1, n_par
                ix = mod(int(dxi(1) * (s_parcels%position(1, n) - lower(1))), nx)
                iy = mod(int(dxi(2) * (s_parcels%position(2, n) - lower(2))), ny)

                ! Cell index of parcel:
                ij = 1 + ix + nx * iy !This runs from 1 to ncell
...
!                 call stop_timer(merge_nearest_timer)
            ...
            do m = 1, nmerge
                is = isma(m)
                x_small = s_parcels%position(1, is)
                y_small = s_parcels%position(2, is)
                ...
                ! Grid point (ix0,iy0) is closest to parcel "is"

                dsqmin = product(extent)
                ic = 0

                ! Loop over 8 cells surrounding (ix0,iy0):
                do iy = iy0-1, iy0
                    do ix = ix0-1, ix0
                        ! Cell index (accounting for x and y periodicity):
                        ij = 1 + mod(nx + ix, nx) + nx * mod(ny + iy, ny)
                        ! Search small parcels for closest other:
                        do k = kc1(ij), kc2(ij)
                            n = node(k)
                            if (n .ne. is) then
                                dely = get_dely(s_parcels%position(2, n), y_small) ! works across periodic edge
                                if (dely*dely < dsqmin) then
                                    delx = get_delx(s_parcels%position(1, n), x_small) ! works across periodic edge
                                    ! Minimise dsqmin
                                    dsq = dely * dely + delx * delx
                                    if (dsq < dsqmin) then
                                        dsqmin = dsq
                                        ic = n
                                    endif
