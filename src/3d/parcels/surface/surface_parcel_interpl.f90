...
module surface_parcel_interpl
    ...
    contains
        ...
        subroutine do_area2grid(s_parcels, n_par, which)
            ...
            do n = 1, n_par
                ...
                do p = 1, 2
                    ...
                    do l = 1, ngp
                        areag(js(l), is(l)) = areag(js(l), is(l)) &
        ...
        subroutine do_par2grid(s_parcels, n_par, which)
            ...
            !$omp& reduction(+:nparg, nsparg, volg)
            do n = 1, n_par
                points = get_ellipse_points(s_parcels%position(:, n), &
                                            s_parcels%B(:, n))
        ...
        ! @param[inout] divs is the parcel divergence source
        ...
        !            (optional)
        subroutine grid2par(vel, divs, vgrad, add)
            double precision,     intent(inout) :: vel(:, :), divs(:), vgrad(:, :)
            ...
            do n = 1, n_parcels
                ...
                do p = 1, 2
                    ...
                    do l = 1, ngp
                        ...
                        vel(:, n) = vel(:, n) + weight * velog(js(l), is(l), :)

                        vgrad(:, n) = vgrad(:, n) + weight * velgradg(js(l), is(l), :)

                        divs(n) = divs(n) + weight * dtend(js(l), is(l))
                    enddo
                enddo
            enddo
            !$omp end do
        ...
        ! @param[inout] divs is the parcel divergence source
        ! @param[inout] vgrad is the parcel strain
        subroutine grid2par_add(vel, divs, vgrad)
            double precision,       intent(inout) :: vel(:, :), divs(:), vgrad(:, :)

            call grid2par(vel, divs, vgrad, add=.true.)
