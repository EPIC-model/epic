...
module ls_rk
    ...
    use surface_parcel_container, only : lo_surf_parcels    &
                                       ...
                                       , n_up_surf_parcels
    use parcel_bc
    use rk_utils, only: get_dBdt, get_time_step, get_surf_dBdt
...
!             call calculate_surface_parcel_diagnostics
        ...
        subroutine ls_rk_substep(dt, step)
            ...
            if (step == 1) then
                ...
                do n = 1, n_lo_surf_parcels
                    lo_surf_parcels%delta_b(:, n) = get_surf_dBdt(lo_surf_parcels%B(:, n),  &
                                                                  lo_surf_parcels%strain(:, n))
                enddo
                !$omp end parallel do

                !$omp parallel do default(shared) private(n)
                do n = 1, n_up_surf_parcels
                    up_surf_parcels%delta_b(:, n) = get_surf_dBdt(up_surf_parcels%B(:, n),      &
                                                                  up_surf_parcels%strain(:, n))
                enddo
                !$omp end parallel do
                ...
                !$omp parallel do default(shared) private(n)
                do n = 1, n_lo_surf_parcels
                    lo_surf_parcels%delta_b(:, n) = lo_surf_parcels%delta_b(:, n)               &
                                                  + get_surf_dBdt(lo_surf_parcels%B(:, n),      &
                                                                  lo_surf_parcels%strain(:, n))
                enddo
                !$omp end parallel do
                ...
                !$omp parallel do default(shared) private(n)
                do n = 1, n_up_surf_parcels
                    up_surf_parcels%delta_b(:, n) = up_surf_parcels%delta_b(:, n)           &
                                                  + get_surf_dBdt(up_surf_parcels%B(:, n),  &
                                                                  up_surf_parcels%strain(:, n))
                enddo
                !$omp end parallel do
            ...
            do n = 1, n_lo_surf_parcels
                lo_surf_parcels%position(:, n) = lo_surf_parcels%position(:, n)     &
                                               + cb * dt * lo_surf_parcels%delta_pos(:, n)

                lo_surf_parcels%vorticity(:, n) = lo_surf_parcels%vorticity(:, n)   &
                                                + cb * dt * lo_surf_parcels%delta_vor(:, n)
                lo_surf_parcels%B(:, n) = lo_surf_parcels%B(:, n) &
                                        + cb * dt * lo_surf_parcels%delta_b(:, n)
            ...
            do n = 1, n_up_surf_parcels
                up_surf_parcels%position(:, n) = up_surf_parcels%position(:, n)     &
                                               + cb * dt * up_surf_parcels%delta_pos(:, n)

                up_surf_parcels%vorticity(:, n) = up_surf_parcels%vorticity(:, n)   &
                                                + cb * dt * up_surf_parcels%delta_vor(:, n)
                up_surf_parcels%B(:, n) = up_surf_parcels%B(:, n) &
                                        + cb * dt * up_surf_parcels%delta_b(:, n)
            ...
            !$omp parallel do default(shared) private(n)
            do n = 1, n_lo_surf_parcels
                lo_surf_parcels%delta_pos(:, n) = ca * lo_surf_parcels%delta_pos(:, n)
                lo_surf_parcels%delta_vor(:, n) = ca * lo_surf_parcels%delta_vor(:, n)
                lo_surf_parcels%delta_b(:, n) = ca * lo_surf_parcels%delta_b(:, n)
            enddo
            !$omp end parallel do

            !$omp parallel do default(shared) private(n)
            do n = 1, n_up_surf_parcels
                up_surf_parcels%delta_pos(:, n) = ca * up_surf_parcels%delta_pos(:, n)
                up_surf_parcels%delta_vor(:, n) = ca * up_surf_parcels%delta_vor(:, n)
                up_surf_parcels%delta_b(:, n) = ca * up_surf_parcels%delta_b(:, n)
            enddo
            !$omp end parallel do
