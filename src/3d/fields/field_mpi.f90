...
!         end subroutine field_halo_accumulate
        ...
        subroutine halo_communication(data)
            ...
            call MPI_Recv(data(box%lo(3):box%hi(3), box%hhi(2)-1:box%hhi(2), box%hlo(1)), southeast_size, &
        ...
        subroutine allocate_buffers
            ...
            allocate(west_buf( box%hi(3)-box%lo(3)+1, box%hi(2)-box%lo(2)+1, 2))
            allocate(east_buf( box%hi(3)-box%lo(3)+1, box%hi(2)-box%lo(2)+1))
            allocate(south_buf(box%hi(3)-box%lo(3)+1, 2, box%hi(1)-box%lo(1)+1))
            allocate(north_buf(box%hi(3)-box%lo(3)+1, box%hi(1)-box%lo(1)+1))

            allocate(northeast_buf(box%hi(3)-box%lo(3)+1))
            allocate(southeast_buf(box%hi(3)-box%lo(3)+1, 2))
            allocate(southwest_buf(box%hi(3)-box%lo(3)+1, 2, 2))
            allocate(northwest_buf(box%hi(3)-box%lo(3)+1, 2))
