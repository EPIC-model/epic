module mpi_halo
    ...
    contains
        ...
        subroutine halo_reset(data)
            ...
            data(box%lo(3):box%hi(3), box%hlo(2):box%hhi(2),   box%hlo(1))              = 0.0d0     ! west halo
            data(box%lo(3):box%hi(3), box%hlo(2):box%hhi(2),   box%hhi(1)-1:box%hhi(1)) = 0.0d0     ! east halo
        ...
        subroutine halo_to_interior_communication
            ...
            call MPI_Isend(east_halo_buf, east_halo_size, MPI_DOUBLE_PRECISION, neighbour%east, &
                          HALO_EAST_TAG, comm%cart, request, comm%err)
            call MPI_Request_free(request)

            call MPI_Recv(west_buf, west_size, MPI_DOUBLE_PRECISION, neighbour%west, &
                          HALO_EAST_TAG, comm%cart, MPI_STATUS_IGNORE, comm%err)



            ! send west halo to east buffer
            call MPI_Isend(west_halo_buf, west_halo_size, MPI_DOUBLE_PRECISION, neighbour%west, &
                           HALO_WEST_TAG, comm%cart, request, comm%err)
            call MPI_Request_free(request)

            call MPI_Recv(east_buf, east_size, MPI_DOUBLE_PRECISION, neighbour%east, &
                          HALO_WEST_TAG, comm%cart, MPI_STATUS_IGNORE, comm%err)



            ! send north halo to south buffer
            call MPI_Isend(north_halo_buf, north_halo_size, MPI_DOUBLE_PRECISION, neighbour%north, &
                           HALO_NORTH_TAG, comm%cart, request, comm%err)
            call MPI_Request_free(request)

            call MPI_Recv(south_buf, south_size, MPI_DOUBLE_PRECISION, neighbour%south, &
                          HALO_NORTH_TAG, comm%cart, MPI_STATUS_IGNORE, comm%err)


            ! send south halo to north buffer
            ! receive north buffer into south halo
            call MPI_Isend(south_halo_buf, south_halo_size, MPI_DOUBLE_PRECISION, neighbour%south, &
                           HALO_SOUTH_TAG, comm%cart, request, comm%err)
            call MPI_Request_free(request)

            call MPI_Recv(north_buf, north_size, MPI_DOUBLE_PRECISION, neighbour%north, &
                          HALO_SOUTH_TAG, comm%cart, MPI_STATUS_IGNORE, comm%err)



            ! send southwest halo to northeast buffer
            call MPI_Isend(southwest_halo_buf, southwest_halo_size, MPI_DOUBLE_PRECISION, neighbour%southwest, &
                           HALO_SOUTHWEST_TAG, comm%cart, request, comm%err)
            call MPI_Request_free(request)

            call MPI_Recv(northeast_buf, northeast_size, MPI_DOUBLE_PRECISION, neighbour%northeast, &
                          HALO_SOUTHWEST_TAG, comm%cart, MPI_STATUS_IGNORE, comm%err)


            ! send northwest halo to southeast buffer
            call MPI_Isend(northwest_halo_buf, northwest_halo_size, MPI_DOUBLE_PRECISION, neighbour%northwest, &
                           HALO_NORTHWEST_TAG, comm%cart, request, comm%err)
            call MPI_Request_free(request)

            call MPI_Recv(southeast_buf, southeast_size, MPI_DOUBLE_PRECISION, neighbour%southeast, &
                          HALO_NORTHWEST_TAG, comm%cart, MPI_STATUS_IGNORE, comm%err)

            ! send southwest buffer to northeast halo
            call MPI_Isend(northeast_halo_buf, northeast_halo_size, MPI_DOUBLE_PRECISION, neighbour%northeast, &
                           HALO_NORTHEAST_TAG, comm%cart, request, comm%err)
            call MPI_Request_free(request)

            call MPI_Recv(southwest_buf, southwest_size, MPI_DOUBLE_PRECISION, neighbour%southwest, &
                          HALO_NORTHEAST_TAG, comm%cart, MPI_STATUS_IGNORE, comm%err)

            ! send northwest buffer to southeast halo
            call MPI_Isend(southeast_halo_buf, southeast_halo_size, MPI_DOUBLE_PRECISION, neighbour%southeast, &
                           HALO_SOUTHEAST_TAG, comm%cart, request, comm%err)
            call MPI_Request_free(request)

            call MPI_Recv(northwest_buf, northwest_size, MPI_DOUBLE_PRECISION, neighbour%northwest, &
        ...
        subroutine interior_to_halo_communication
            ...
            call MPI_Isend(west_buf, west_size, MPI_DOUBLE_PRECISION, neighbour%west, &
                           HALO_EAST_TAG, comm%cart, request, comm%err)
            call MPI_Request_free(request)

            ! receive west buffer to east halo (left to right)
            call MPI_Recv(east_halo_buf, east_halo_size, MPI_DOUBLE_PRECISION, neighbour%east, &
                          HALO_EAST_TAG, comm%cart, MPI_STATUS_IGNORE, comm%err)
            ...
            call MPI_Isend(east_buf, east_size, MPI_DOUBLE_PRECISION, neighbour%east, &
                           HALO_WEST_TAG, comm%cart, request, comm%err)
            call MPI_Request_free(request)

            ! receive east buffer into west halo (right to left)
            call MPI_Recv(west_halo_buf, west_halo_size, MPI_DOUBLE_PRECISION, neighbour%west, &
                          HALO_WEST_TAG, comm%cart, MPI_STATUS_IGNORE, comm%err)



            ! send south buffer to north halo
            call MPI_Isend(south_buf, south_size, MPI_DOUBLE_PRECISION, neighbour%south, &
                           HALO_NORTH_TAG, comm%cart, request, comm%err)
            call MPI_Request_free(request)

            ! receive south buffer into north halo
            call MPI_Recv(north_halo_buf, north_halo_size, MPI_DOUBLE_PRECISION, neighbour%north, &
                          HALO_NORTH_TAG, comm%cart, MPI_STATUS_IGNORE, comm%err)
            ...
            call MPI_Isend(north_buf, north_size, MPI_DOUBLE_PRECISION, neighbour%north, &
                           HALO_SOUTH_TAG, comm%cart, request, comm%err)
            call MPI_Request_free(request)

            ! receive north buffer into south halo
            call MPI_Recv(south_halo_buf, south_halo_size, MPI_DOUBLE_PRECISION, neighbour%south, &
                          HALO_SOUTH_TAG, comm%cart, MPI_STATUS_IGNORE, comm%err)
            ...
            call MPI_Isend(northeast_buf, northeast_size, MPI_DOUBLE_PRECISION, neighbour%northeast, &
                           HALO_SOUTHWEST_TAG, comm%cart, request, comm%err)
            call MPI_Request_free(request)

            ! receive northeast buffer into southwest halo
            call MPI_Recv(southwest_halo_buf, southwest_halo_size, MPI_DOUBLE_PRECISION, neighbour%southwest, &
                          HALO_SOUTHWEST_TAG, comm%cart, MPI_STATUS_IGNORE, comm%err)
            ...
            call MPI_Isend(southeast_buf, southeast_size, MPI_DOUBLE_PRECISION, neighbour%southeast, &
                           HALO_NORTHWEST_TAG, comm%cart, request, comm%err)
            call MPI_Request_free(request)

            ! receive southeast buffer into northwest halo
            call MPI_Recv(northwest_halo_buf, northwest_halo_size, MPI_DOUBLE_PRECISION, neighbour%northwest, &
                          HALO_NORTHWEST_TAG, comm%cart, MPI_STATUS_IGNORE, comm%err)
            ...
            ! send southwest buffer to northeast halo
            call MPI_Isend(southwest_buf, southwest_size, MPI_DOUBLE_PRECISION, neighbour%southwest, &
                           HALO_NORTHEAST_TAG, comm%cart, request, comm%err)
            call MPI_Request_free(request)

            ! receive southwest buffer into northeast halo
            call MPI_Recv(northeast_halo_buf, northeast_halo_size, MPI_DOUBLE_PRECISION, neighbour%northeast, &
                           HALO_NORTHEAST_TAG, comm%cart, MPI_STATUS_IGNORE, comm%err)
            ...
            ! send northwest buffer to southeast halo
            call MPI_Isend(northwest_buf, northwest_size, MPI_DOUBLE_PRECISION, neighbour%northwest, &
                           HALO_SOUTHEAST_TAG, comm%cart, request, comm%err)
            call MPI_Request_free(request)

            ! receive northwest buffer into southeast halo
            call MPI_Recv(southeast_halo_buf, southeast_halo_size, MPI_DOUBLE_PRECISION, neighbour%southeast, &
                          HALO_SOUTHEAST_TAG, comm%cart, MPI_STATUS_IGNORE, comm%err)
