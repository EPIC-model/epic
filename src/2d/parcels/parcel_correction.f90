...
module parcel_correction
    ...
    use parcel_interpl, only : bilinear, bilinear_old, ngp, vol2grid
    use parcel_bc
    use omp_lib
    ...
    subroutine apply_laplace
        ...
        do n = 1, n_parcels
            ...
            parcels%position(2, n) = parcels%position(2, n)             &
                                   + sum(weights * wd(js:js+1, is:is+1))
    ...
    subroutine apply_gradient(prefactor, max_compression)
        ...
        do n = 1, n_parcels
            ...
            x1_fpos=sum(weights(:,1)) ! fractional position along x1
            x2_fpos=sum(weights(1,:)) ! fractional position along x2

            shift_x1= - prefactor*dx(1)*x1_fpos*(one-x1_fpos)*(&
                        (one-x2_fpos)*(phi(js, is+1)-phi(js, is))  &
                      +     (x2_fpos)*(phi(js+1, is+1)-phi(js+1, is)))
            ...
            shift_x2= - prefactor*dx(2)*x2_fpos*(one-x2_fpos)*(&
                        (one-x1_fpos)*(phi(js+1, is)-phi(js, is)) &
                      +     (x1_fpos)*(phi(js+1, is+1)-phi(js, is+1)))
