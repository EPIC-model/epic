...
write(*,*) ' We assume any parcel volume is equally probable.'
write(*,*) ' Enter V_max/V_min:'
read(*,*) vrange
...
! Re-normalise so that sum(v) = ncell = nx*ny*nz:
fac=dble(ncell)/sum(v(1:n))
v(1:n)=fac*v(1:n)

!---------------------------------------------------------------------
! Write parcel positions and volume fractions (of grid cell volume):
open(80,file='ini_parcels.asc',status='replace')
write(80,*) n
do i=1,n
  write(80,*) x(i),y(i),z(i),v(i)
enddo
close(80)
