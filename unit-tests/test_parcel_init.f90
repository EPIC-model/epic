...
! =============================================================================
program test_parcel_init !FIXME
    use unit_test
    ...
    integer, parameter :: nbgy = 2, nbgz = nbgy
#ifdef ENABLE_3D
    integer, parameter :: nbgx = 2
#else
    integer, parameter :: nbgx = 1
#endif
    ...
    do ix = 0, nx-1
        ...
        do iy = 0, ny-1
            ...
            do iz = 0, nz
                zg = dx(2) * dble(iz)
                argz = facz * zg
                tbuoyg(iz, iy, ix) = dexp(argz) * dsin(argy+one) / (one + f12 * dcos(argy)) &
                                   * f12 * dble(ix)
            enddo
        enddo
    enddo
                ...
                do mz = 1,nbgz
                    do my = 1, nbgy
                        do mx = 1,nbgx
                            i = i + 1
#ifdef ENABLE_3D
                            parcels%position(i, 1) = lower(1) &
                                                   + dx(1) * (dble(ix) + dxf * (dble(mx) - f12))
                            parcels%position(i, 2) = lower(2) &
                                                   + dx(2) * (dble(iy) + dyf * (dble(my) - f12))
                            parcels%position(i, 3) = lower(3) &
                                                   + dx(3) * (dble(iz) + dzf * (dble(mz) - f12))
#else
                            parcels%position(i, 1) = lower(1) &
                                                   + dx(1) * (dble(iy) + dyf * (dble(my) - f12))
                            parcels%position(i, 2) = lower(2) &
                                                   + dx(2) * (dble(iz) + dzf * (dble(mz) - f12))
#endif
                            parcels%volume(i) = v0
                            parcels%B(i, 2) = zero
