...
!
!   This unit test checks a = b - c across y MPI boundaries. The small parcels
!   'a' and 'c' should want to merge with the small parcel 'b'.
! =============================================================================
program test_mpi_nearest_7
    use unit_test
    ...
    contains
        ...
        subroutine cell_placement(l, i, j, k)
            ...
            ! small parcel b
            parcels%position(1, l) = x
            parcels%position(2, l) = y - dx(1) * 0.4d0
            parcels%position(3, l) = z
            parcels%volume(l) = 0.9d0 * vmin
            parcels%theta(l) = l + world%rank * 100
            ...
            ! small parcel a
            parcels%position(1, l) = x
            parcels%position(2, l) = y - dx(1) * 0.3d0
            parcels%position(3, l) = z
            parcels%volume(l) = 0.9d0 * vmin
            parcels%theta(l) = l + world%rank * 100
...
end program test_mpi_nearest_7
