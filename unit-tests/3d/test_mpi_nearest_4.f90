...
!
!   This unit test checks a - B across y MPI boundaries.
! =============================================================================
program test_mpi_nearest_4
    use unit_test
    ...
    if (world%rank == world%root) then
        passed = (passed .and. (check_array(1) == parcels%total_num) .and. (check_array(2) == 100))
    ...
    contains
        ...
        subroutine cell_placement(l, i, j, k)
            ...
            parcels%position(2, l) = y + 0.1d0 * dx(2)
            ...
            ! big parcel
            parcels%position(1, l) = x
            parcels%position(2, l) = y - 0.42d0 * dx(2)
            parcels%position(3, l) = z
            parcels%volume(l) = 1.1d0 * vmin
            parcels%buoyancy(l) = l + world%rank * 100
            l = l + 1
...
end program test_mpi_nearest_4
